...
export default function App() {
  ...
  const updateItem = (updatedItem: FoodItem) => {
    ...
    showToast(`${updatedItem.name} を更新しました`);
  };
...
const ItemCard = ({ item, deleteItem, onAddToShoppingList, isLowStock, threshold, onEdit }: { item: FoodItem, deleteItem: (id: string) => void, onAddToShoppingList: (name: string, quantity?: number, unit?: string) => void, isLowStock?: boolean, threshold?: number, onEdit?: (item: FoodItem) => void }) => {
  ...
  return (
    <div onClick={() => onEdit && onEdit(item)} className={`p-4 rounded-xl border flex items-center justify-between shadow-sm transition-all hover:shadow-md mb-3 cursor-pointer ${getStatusColor(item.expiryDate, isLowStock, item.quantity)}`}>
      ...
      <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1"><label className="text-xs font-bold text-gray-500">商品名</label><input type="text" className="w-full p-2 border-b border-gray-300 focus:border-green-500 outline-none font-bold" value={data.name} onChange={(e) => setData({ ...data, name: e.target.value, categorySmall: e.target.value })} /></div>
          </div>
